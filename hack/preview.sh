...
# Copy utils functions in a temporary file
if [ ! -d ".tmp" ]; then mkdir -p .tmp; fi
curl https://raw.githubusercontent.com/rsoaresd/service-provider-integration-operator/SVPI-518/hack/utils.sh -o $(realpath .tmp)/utils.sh

# Init Vault
curl https://raw.githubusercontent.com/rsoaresd/service-provider-integration-operator/SVPI-518/hack/vault-init.sh | VAULT_PODNAME='vault-0' VAULT_NAMESPACE='spi-vault' bash -s

# Init SPI
curl https://raw.githubusercontent.com/rsoaresd/service-provider-integration-operator/SVPI-518/hack/spi-init.sh | VAULT_PODNAME='vault-0' VAULT_NAMESPACE='spi-vault' bash -s

# Init Remote Secret
curl https://raw.githubusercontent.com/rsoaresd/service-provider-integration-operator/SVPI-518/hack/remote-secret-init.sh | VAULT_PODNAME='vault-0' VAULT_NAMESPACE='spi-vault' bash -s
